...
export default function AdminEditPackagePage({ params }: PageProps) {
  ...
  const handleCaptureSnapshot = async () => {
    ...
    if (!canvas) {
      alert('Could not find 3D room canvas element. Please wait for the 3D room to finish loading.');
      return;
    }
    ...
    try {
      ...
      alert(`Failed to capture 3D room snapshot: ${err.message}`);
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pkgData = await packageService.getPackage(packageId);
        setName(pkgData.name);
        setDescription(pkgData.description || '');
        setDiscountPercent(pkgData.discountPercent);
        setCoverImage(pkgData.imageUrl || '');

        // 3. Load initial package items into 3D Store
        const STATIC_BASE = apiUrl.replace('/api', '');
        const itemsMap = new Map(catalogData.map((item: any) => [item.itemId, item]));
        ...
        (pkgData.items || []).forEach((pi: any) => {
          ...
          if (isTile) {
            ...
            const randId = 'admin_edit_preview_' + matchedItem.itemId;
            mappedItems.push({
              id: randId,
              type,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    (placedItems || []).forEach(placedItem => {
      ...
      if (matched) {
        ...
        const fallbackId = placedItem.id ? placedItem.id.replace('admin_edit_preview_', '') : null;
    ...
    try {
      ...
      alert(`Failed to save package changes: ${err.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4 font-sans">
        ...
        <p className="text-sm text-gray-500 font-light">Loading package editor...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 font-sans max-w-[1700px] mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* LEFT COLUMN: Metadata & Real-time Financial Calculator (4 / 12 Width) */}
        ...
        {/* RIGHT COLUMN: Full Interactive 3D Room Suite Designer Canvas & Side Menu (8 / 12 Width) */}
        <div className="lg:col-span-8 bg-white border border-gray-200 shadow-sm rounded-xl p-4 space-y-3 sticky top-4">
